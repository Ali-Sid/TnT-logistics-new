...
function TopSecondPanel({ handleOpen, selectedItem, totalCount }) {
    ...
    return (
        <div style={{ borderBottom: '1px solid rgba(0, 0, 0, 0.12)', boxShadow: '0px 1px 0.5px -0.5px rgba(0,0,0,0.2), 0px 0.5px 0.5px 0px rgba(0,0,0,0.14), 0px 0.5px 1px 0px rgba(0,0,0,0.12)' }}>
            ...
            <Box sx={{display: "flex", flexDirection: "row"}}>
                <Box sx={{ marginLeft: "10px", padding: "5px 10px 5px 10px", border: "1px solid #d32f2f", backgroundColor: "#d32f2f", color: "#fff", borderRadius: "5px", display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", width: "auto", height: "16px" }}>
                    <Typography variant='h5' fontWeight={400} fontSize={20}>{totalCount}</Typography>
                </Box>
                <Typography align='left' pl={1} variant="h6" color='#5c5c5c' noWrap>
                    {selectedItem ? selectedItem.PItemName : "Items List"}
                </Typography>
            </Box>
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", mt: 0, ml: 2, mr: 2 }}>
                {/* <Button onClick={handleOpen} variant="contained" color="error">Add New</Button> */}
                <Button startIcon={<AddBoxIcon />} onClick={handleOpen} variant='text' color='error' sx={{ textTransform: 'none', fontSize: "16px" }}>Boxes Produced</Button>
