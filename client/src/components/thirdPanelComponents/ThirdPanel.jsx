...
const ThirdPanel = ({ selectedItemList, group, allItems, onItemClick, selectedItem }) => {
  ...
  const [associateResources, setAssociateResources] = useState([]);
  ...
  const fetchAssociateResources = async () => {
    try {
      const response = await axios.get('http://localhost:3000/getAssociateResources');
      // Assuming the response data is an array of resources
      setAssociateResources(response.data);
    } catch (error) {
      console.error('Error fetching AssociateResources:', error);
    }
  };
  ...
  return (
    <div>
        ...
        <div>
          {selectedItemList && (
            <div style={{ textAlign: "left", paddingLeft: "10px" }}>
              ...
              <p>Name: {selectedItemList.Name}</p>
      ...
      <div style={{ width: "70%" }}>
        <div>
          {associateResources.map((resource, index) => (
            <div key={index}>
              <p>Resource Name: {resource.AssociateResName}</p>
              <p>Resource Description: {resource.MainResName}</p>
              {/* Render other resource details as needed */}
            </div>
          ))}
        </div>
